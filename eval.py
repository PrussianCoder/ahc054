...
class SingleTest:
    ...
    def test(self):
        ...
        cp = subprocess.Popen(
            f"exec {TESTER} {self.testee} < {self.input} > {self.output}",
            shell=True,
            env=self.env,
        ...
        if score is None:
            if SCORER:
                ...
                cp = subprocess.run(
                    f"{SCORER} {self.input} {self.output}",
                    shell=True,
                    timeout=10,
                ...
                if score is None:
                    score = get_score_from_last_logs(cp.stdout.rstrip())
                if (
                    score is None
                ):  # 標準出力と標準エラー出力の両方にスコアが無ければエラー表示
...
class Objective:
    ...
    def __init__(self, args, dummy_test=False):
        ...
        if dummy_test:
            ...
            self.dbg_(
                f"{num_cpus=} max_concurrent_workers={self.max_concurrent_workers}"
            )
    ...
    def __call__(self, trial=None):
        ...
        for id in self.test_ids:
            ...
            if len(workers) >= self.max_concurrent_workers:
                ...
                self.eval_result_(
                    trial, finished, raw_results, results
                )  # 結果集計と枝刈り
        self.eval_result_(
            trial, workers, raw_results, results
        )  # 残った結果集計と枝刈り
        self.dbg_("done.")
        # 後処理
        duration_total = time.time() - start_time
        if self.debug:
            self.print_score_(results, duration_total)
        if self.standings:
            self.add_standings_(results)
            ...
            for kind, value in values.items():
                ...
                if kind == "int":
                    ...
                    if step is None:
                        ...
                        env[name] = str(
                            trial.suggest_float(name, *value[:2], step=step)
                        )
        return env
    ...
    def eval_result_(self, trial, workers, raw_results, results):
        ...
        while (
            len(results) < len(self.test_ids)
            and (id := self.test_ids[len(results)]) in raw_results
        ):
            results.append(Result(id, results.dirs, *raw_results[id]))
            if not trial:
                continue
            trial.report(
                results.logscore_sum, len(results)
            )  # Optunaに結果を報告して枝刈りする
    ...
    def print_score_(self, results, duration_total):
        ...
        self.dbg_(
            f"Total score: {results.score_sum} (av. {int(results.score_sum / len(results.items))}) log: {results.logscore_sum:.3f} (max time: {results.duration_max:.3f}s)"
        )
        self.dbg_(
            f"Total time: {duration_total:.3f}s ({duration_total / len(results):.3f}s/test)"
        ...
        if not os.path.isfile(input_file):
            ...
            with open(input_file, "w") as f:
                ...
                for result in results.items:
                    f.write(
                        f'{result.id:04},0,{",".join(map(str, result.features[:len(FEATURES)]))}\n'
                    )
...
def parser():
    ...
    parser.add_argument(
        "-v", "--visible", help="visible test stderr.", action="store_true"
    )
    parser.add_argument(
        "-o", "--optuna", type=int, help="optuna n_trials, forced --silent", default=0
    )
    parser.add_argument(
        "--dir",
        nargs="*",
    ...
    parser.add_argument(
        "--testee", type=str, help="the char in bin name replaced from 'a'", default="a"
    )
...
def compile(args):
    ...
    if testee_compile is None:
        return False
    if (
        os.path.isfile(testee)
        and os.stat(testee_source).st_mtime < os.stat(testee).st_mtime
    ):
        return False
    ...
    Objective(
        args, dummy_test=True
    )()  # 初回実行は遅いので、計測前に1回だけダミー実行しておく
    return True
...
def main():
    ...
    study = optuna.create_study(
        ...
        storage="sqlite:///./tools/out/optuna.db",
        load_if_exists=True,
    )
    # enque_trialの値を設定
    best_params = {}
