...
class BoardSimulatorOptimized:
    ...
    def _pos_to_bit(self, i: int, j: int) -> int:
        """(i, j)座標を単一ビット位置に変換"""
        return i * self.n + j

    def _is_revealed_bit(self, i: int, j: int) -> bool:
        """ビットセットから位置(i,j)が明らかになっているか確認"""
        bit_pos = self._pos_to_bit(i, j)
        return (self.revealed_bits >> bit_pos) & 1

    def _set_revealed_bit(self, i: int, j: int):
        """位置(i,j)を明らかにする"""
        bit_pos = self._pos_to_bit(i, j)
        self.revealed_bits |= (1 << bit_pos)

    def _is_passable_bit(self, i: int, j: int) -> bool:
        """通行可能かチェック（ビットセット版）"""
        bit_pos = self._pos_to_bit(i, j)
        return (self.passable_bits >> bit_pos) & 1
        ...
        while queue:
            ...
            for di, dj in self.dij:
                ...
                if (
                    ...
                    and (
                        not self._is_revealed_bit(i2, j2)
                        or self._is_passable_bit(i2, j2)
                    )
                ):
    ...
    def _reveal_from_position(self, pos: tuple[int, int]) -> bool:
        ...
        for di, dj in self.dij:
            ...
            while True:
                ...
                if not self._is_revealed_bit(i2, j2):
                    self._set_revealed_bit(i2, j2)
                    self.new_revealed.append((i2, j2))
                    changed = True

                # 障害物で視線が遮られる
                if not self._is_passable_bit(i2, j2):
                    break
    ...
    def simulate(self, board: Board) -> int:
        ...
        # ビットセットで明らかになったセルを管理
        self.revealed_bits = 0
        self._set_revealed_bit(self.start_i, self.start_j)
    ...
    def _step(
        ...
        if self._is_revealed_bit(self.t[0], self.t[1]):
        ...
        if self.target == (-1, -1) or (
            self.target != self.t and self._is_revealed_bit(self.target[0], self.target[1])
        ):
            ...
            while True:
                if self.q:
                    target = self.q.pop()
                    if not self._is_revealed_bit(target[0], target[1]) and self.dist[target[0]][
                        target[1]
                    ] != float("inf"):
                        self._change_target(target)
                        break
